...
public sealed class UpdateUkrainianStorageProductsAvailabilityActor : ReceiveActor {
    private readonly IDbConnectionFactory _connectionFactory;

    public UpdateUkrainianStorageProductsAvailabilityActor(IDbConnectionFactory connectionFactory) {
        _connectionFactory = connectionFactory;

        Receive<UpdateUkrainianStorageProductsAvailabilityMessage>(UpdateAvailability);
    }

    private void UpdateAvailability(UpdateUkrainianStorageProductsAvailabilityMessage message) {
        string logFilePath = $"{ConfigurationManager.EnvironmentRootPath}\\Logs\\update_availability_uk_log.txt";

        try {
            const string updateSqlScriptPath = @"D:\To Sync Data\upload_ua\upload.sql";
            ...
            if (!File.Exists(updateSqlScriptPath)) {
                ...
                using (IDbConnection connection = _connectionFactory.NewSqlConnection()) {
                    connection.Execute("UPDATE ProductAvailability SET Amount = 0 WHERE [ProductAvailability].StorageID = 1 AND [ProductAvailability].Deleted = 0");
                }
            ...
            WHERE [ProductAvailability].StorageID = 1
            ...
            unixtime  nvarchar(100) 
                                ...
                                builder.Append(
                                    "INSERT INTO dbo.#TempProducts (id,shop_category_id,part_number,name,description,remain, weight, size, main_original_number,status, unixtime) VALUES ");
